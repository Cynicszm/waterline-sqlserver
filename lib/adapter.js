...
var runJoins = require('waterline-cursor');
...
function logError() {
  console.error("\n\x1b[1m\x1b[31m!! \x1b[0m\x1b[1mMSSQL Server adapter error:\x1b[0m\n");
...
module.exports = (function () {
  ...
  var adapter = {
    ...
    handleQueryError : function(mssqlConnect, collection, err, cb) {
        // Check for request error:
        // Manually parse the SQL Server error response and extract the relevant bits,
        ...
        //Query the information_schema view INFORMATION_SCHEMA.CONSTRAINT_COLUMN_USAGE to find the associated column
        //Build the return message        
        if (err && err.message)
        {  
          var matches = err.message.match(/^Violation of (.*?) constraint '([^']*)'\.[^']*'([^']*)'[^(]*\(([^)]*)\)/);

          /*
            Match 1 - type of violation e.g. UNIQUE KEY
            March 2 - constraint name
            Match 3 - schema.table
            Match 4 - value
          */    
          ...
          if (matches && matches.length)
          {
            ...
            //found a match - check that we have a unique key match and then build return
            if (matches[1] === 'UNIQUE KEY')
            {
              var statement = "SELECT COLUMN_NAME as columnName from INFORMATION_SCHEMA.CONSTRAINT_COLUMN_USAGE where CONSTRAINT_NAME = '" + matches[2] + "'";
              var request = new mssql.Request(mssqlConnect);

              request.query(statement)
                .then(function (results){
                  
                  //Get the column associated with the constraint
                  var columnName = results[0].columnName;                  

                  //Format a message to be used in WLValidation Error.
                  //Includes the column that failed as well as the value which caused the error.
                  var formattedErr = {};
                  formattedErr.model = collection;
                  formattedErr.code = 'E_UNIQUE';
                  formattedErr.invalidAttributes = {};
                  formattedErr.invalidAttributes[columnName] = 
                          [{
                          value: matches[4],
                          rule: 'unique'
                          }]; 
    ...
    teardown: function (conn, cb) {
      ...
      if (!conn) {
        _.each(connections, function (c) {
          if (c.persistent && c.mssqlConnection) {
            c.mssqlConnection.close();
          } else {
      ...
      if (connections[conn].persistent) {
        ...
        _.each(connections[conn], function (handle) {
          if (handle.mssqlConnection) {
            handle.mssqlConnection.close();
          }
        });
      }
    ...
    describe: function (connection, collection, cb) {
      ...
      adapter.connectConnection(connection, function __DESCRIBE__(err, uniqId) {
        ...
        request.query(statement, function (err, recordset) {
          ...
          if (!connections[connection].persistent && mssqlConnect) {
            mssqlConnect.close();
          }
      ...
      adapter.connectConnection(connection, function __FIND__(err, uniqId) {
        ...
        if (_.isArray(data) && data.length) {
          for (var i = 0, l = data.length; i < l; i++) {
            query = query.replace(new RegExp('\\$' + String(i + 1), 'g'), escape(data[i]));
          }
        }
        ...
        request.query(query, function (err, recordset) {
          ...
          if (connections[connection] && !connections[connection].persistent && mssqlConnect) {
            mssqlConnect.close();
          }
    ...
    create: function (connection, collection, values, cb) {
      ...
      if (identityInsert) {
        statement = 'SET IDENTITY_INSERT [' + collection + '] ON; ' + statement + 'SET IDENTITY_INSERT [' + collection + '] OFF;';
      }
      ...
      adapter.connectConnection(connection, function __CREATE__(err, uniqId) {
        ...
        request.query(statement, function (err, recordsets) {
          if (err) {
            logError(err); 
            return adapter.handleQueryError(mssqlConnect, collection, err, cb);         
          }
